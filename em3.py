...
class EM_algorithm:
    ...
    def fit(self, data, belonging_, em_params, base_name, weights):
        ...
        plt.savefig(f"{base_name}_em5_likelihood.png",bbox_inches='tight')
        plt.close()
...
def process_images_probabilistic(testing_folder, em_params):
    ...
    for image_file in os.listdir(images_folder):
        ...
        csf_path = os.path.join(mni_folder,base_name, "mni_CSF_transformed.nii.gz")
        gm_path = os.path.join(mni_folder, base_name,"mni_GM_transformed.nii.gz")
        wm_path = os.path.join(mni_folder, base_name,"mni_WM_transformed.nii.gz")

        csf_data = nib.load(csf_path).get_fdata()
        gm_data = nib.load(gm_path).get_fdata()
        wm_data = nib.load(wm_path).get_fdata()

        probabilities = np.array([csf_data, gm_data, wm_data])
        max_prob = np.max(probabilities, axis=0)
        tissue_map = np.where(max_prob > 0.3, np.argmax(probabilities, axis=0) + 1, 0)
        tissue_map = au.apply_mask(tissue_map, mask_data)

        weights = calculate_weights_from_tissue_maps(tissue_map)

        relevant_data, nonzero_indices = eh.create_data_nonzero(normalized_image_data)

        aligned = eh.align_probs_with_nonzero_data(probabilities, nonzero_indices)

        tissue_stats = eh.calculate_tissue_intensity_stats(normalized_image_data,tissue_map)

        em = EM_algorithm(n_components=3, method='km', mvm_method='mvn', max_iterations=150, tolerance=1e-5)
        ...
        au.plot_comparison(final_array, label_data, title1="EM Segmented Tissue", title2="Ground Truth", 
                        slice_nums=(136, 142, 163), save_filename=f"{base_name}_em5kmeans.png")
